...
if($data = Cache::read('product_all')){
	$lefty = in_array($main_category, array('Accesorios de Pesca y Deportivos','Productos de Energía')) ? ' lefty':'';
	echo $html->div('lists '.Inflector::slug($main_category).$lefty);
	...
	switch($main_category){
		...
		case 'Aquamotos':
			echo
				...
				'</div>',
				$html->div('column'),
					$util->mklist($data['Recreaci&oacute;n'],'Recreación'),
