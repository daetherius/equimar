...
class CartComponent extends Object {
	...
	function add2cart($item_id = false,$type_id = false){
		if($item_id){
			...
			$item['Type'] = false;
			if($type_id && $type = $this->Product->Type->find_(array($type_id,'contain'=>false,'fields'=>array('id','precio')))){
				$item['Type'] = $type['Type'];
			}
	...
	function updateqty(){
		if($this->controller->data){
			$response = '';
			foreach($this->controller->data['Product'] as $item_id => $item){
				if($this->Session->check('cart.items.'.$item_id)){
					$this->Session->write('cart.items.'.$item_id.'.qty',(int)$item['qty']);
				}
			}
			$this->response('Carrito actualizado');
		}
	}
